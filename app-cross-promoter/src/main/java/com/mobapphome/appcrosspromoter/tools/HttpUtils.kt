...
object HttpUtils {
    ...
    fun jsonToProgramList(jsonStr: String?): MAHRequestResult {
        ...
        try {
            ...
            // Log.i(ACPController.LOG_TAG_MAH_ADS, "Programs size = " + programs.length());
            for (i in 0..programs.length() - 1) {
                try {
                    ...
                    //Log.i(ACPController.LOG_TAG_MAH_ADS, "Added = " + name);
    ...
    fun requestProgramsVersion(context: Context, url: String): Int {
        ...
        val jsonStr = doc.body().text()
        //Log.i(ACPController.LOG_TAG_MAH_ADS, jsonStr);
