...
fun showMarket(context: Context, pckgName: String) {
    ...
    try {
        ...
        Toast.makeText(context, context.getString(R.string.acp_play_service_not_found), Toast.LENGTH_LONG).show()
        Log.e(Constants.LOG_TAG_MAH_ADS, context.getString(R.string.acp_play_service_not_found) + e.message)
    }
...
private fun programSelect(programsSource: MutableList<Program>, programsSelectedLocal: MutableList<Program>) {
    ...
    while (programsSource.size > 0 && programsSelectedLocal.size < 2) {
        //Log.i(ACPController.LOG_TAG_MAH_ADS, "DBRequester prog filtered count  = " + programsFiltered.size());
        val randomIndex = random.nextInt(programsSource.size)
        //Log.i(ACPController.LOG_TAG_MAH_ADS, "DBRequester random number = " + randomIndex);
